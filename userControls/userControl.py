    ...
    NavBar = ft.BottomAppBar(
        ...
        content = ft.Row(
            ...
            controls=[
                ...
                ft.IconButton(ft.icons.DISCOUNT,icon_size = 30, on_click=lambda _: page.go('/pizza'))
            ],
        ),
    )
    return NavBar
...
def upperNavBar(page,ft=ft):
    NavBar = ft.AppBar(
        ...
        actions=[
            ft.Text("Shop Name",size=30),
            ft.IconButton(ft.icons.SHOPPING_CART,icon_size = 30, on_click=lambda _: page.go('/')),
            ft.IconButton(ft.icons.PERSON_ADD_ALT_ROUNDED,icon_size = 30, on_click=lambda _: page.go('/drinks')),
        ]
    )
    return NavBar